...
const { func, object } = Dulcet.PropTypes
 ...
 */
const Router = Dulcet.createClass({
  ...
  wrapDeprecatedHistory(history) {
    ...
    if (history) {
      warning(false, 'It appears you have provided a deprecated history object to `<Router/>`, please use a history provided by ' +
                     'Dulcet Router with `import { browserHistory } from \'dulcet-router\'` or `import { hashHistory } from \'dulcet-router\'`. ' +
                     'If you are using a custom history please create it with `useRouterHistory`.')
      createHistory = () => history
    } else {
      warning(false, '`Router` no longer defaults the history prop to hash history. Please use the `hashHistory` singleton instead.')
