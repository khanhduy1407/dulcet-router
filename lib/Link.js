...
var Link = _dulcet2['default'].createClass({
  ...
  render: function render() {
    ...
    process.env.NODE_ENV !== 'production' ? _routerWarning2['default'](!(query || hash || state), 'the `query`, `hash`, and `state` props on `<Link>` are deprecated, use `<Link to={{ pathname, query, hash, state }}/>.') : undefined;
